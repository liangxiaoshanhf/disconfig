    ...
    public Object decideAccess(ProceedingJoinPoint pjp, DisconfFileItem disconfFileItem) throws Throwable {
        ...
        if (DisClientConfig.getInstance().ENABLE_DISCONF) {
            ...
            if (field != null) {
              ...
              //获取模块名组装成xxx.xx，例如user.name,结合springboot
                 String keyName = disconfFileItem.name();
                if(keyName.contains("?")){
                	String moduleName =DisconfMgr.getInstance().getProperties(DisconfModul.MODULE_NAME);
                	if(!StringUtils.isEmpty(moduleName)){
                		keyName=keyName.replace("?", moduleName);
                	}
                }
                
				Object ret = disconfStoreProcessor.getConfig(disconfFile.filename(), keyName);
                if (ret != null) {
