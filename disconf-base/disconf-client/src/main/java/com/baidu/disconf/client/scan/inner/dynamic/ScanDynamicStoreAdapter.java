...
import com.baidu.disconf.client.aop.AopTargetUtils;
...
public class ScanDynamicStoreAdapter {
    ...
    private static void processFiles(Map<DisconfKey, List<IDisconfUpdate>> inverseMap,
        ...
        /*	if(curClass.isInterface()){
        		Map<String, ?> beanOfType = DisconfMgr.getInstance().getBeansOfType(curClass);
        	    if(!beanOfType.isEmpty()){
        	    	for(Object bean:beanOfType.values()){
        	    		 DisconfFile disconfFile = AopTargetUtils.getTarget(bean).getClass().getAnnotation(DisconfFile.class);
